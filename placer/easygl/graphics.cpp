...
#define DRAW_ERROR()                                                           \
    {                                                                          \
        char msg[BUFSIZE];                                                     \
        sprintf(msg,                                                           \
                "Error %i: Couldn't draw graphics object on line %d of "       \
                "graphics.c\n",                                                \
                GetLastError(), __LINE__);                                     \
        MessageBox(NULL, msg, NULL, MB_OK);                                    \
        exit(-1);                                                              \
 ...
 * the graphics area (user routine). win, hwnd:  X11 and Win32 data pointer
 * to the window, respectively. button_type: indicates if this button displays
...
static void invalidate_screen(void) {
    /* Tells the graphics engine to redraw the graphics display since
...
static void force_setcolor(int cindex) {
    ...
    if (disp_type == SCREEN) {
        ...
        if (!DeleteObject(hGraphicsPen))
            DELETE_ERROR();
        hGraphicsPen =
            ExtCreatePen(PS_GEOMETRIC | win_linestyle | PS_ENDCAP_FLAT,
...
static void force_setlinestyle(int linestyle) {
    ...
    if (disp_type == SCREEN) {
#ifdef X11
        XSetLineAttributes(display, current_gc, currentlinewidth,
...
static void force_setlinewidth(int linewidth) {
    ...
    if (disp_type == SCREEN) {
#ifdef X11
        XSetLineAttributes(display, current_gc, linewidth,
...
#else
        button[bnum].hwnd = CreateWindow(
            TEXT("button"), TEXT(button[bnum].text),
...
void create_button(const char* prev_button_text, const char* button_text,
    ...
    button =
        (t_button*)my_realloc(button, (num_buttons + 1) * sizeof(t_button));
...
void init_graphics(const char* window_name, int cindex) {
    ...
    toplevel = XCreateSimpleWindow(display, RootWindow(display, screen_num), x,
                                   y, top_width, top_height, border_width,
    ...
    // hGraphicsPen = CreatePen(win32_line_styles[SOLID], 1,
    // win32_colors[BLACK]);
...
#define ON 0
    ...
    while (1) {
        ...
        case KeyPress:
            ...
            if (get_keypress_input) {
                ...
                length = XLookupString(&report.xkey, keyb_buffer, max_bytes,
    ...
    while (!ProceedPressed && GetMessage(&msg, NULL, 0, 0)) {
        ...
        if (msg.message ==
            WM_CHAR) { // only the top window can get keyboard events
            msg.hwnd = hMainWnd;
        }
        DispatchMessage(&msg);
    }
 ...
 * Coordinates are in world (user) space.                         */
    ...
    if (disp_type == SCREEN) {
        ...
        XDrawLine(display, toplevel, current_gc, xcoord(x1), ycoord(y1),
                  xcoord(x2), ycoord(y2));
#else /* Win32 */
...
void drawrect(float x1, float y1, float x2, float y2) {
    ...
    if (disp_type == SCREEN) {
        ...
        if (!(hOldPen = (HPEN)SelectObject(hGraphicsDC, hGraphicsPen)))
            SELECT_ERROR();
        if (!(hOldBrush = (HBRUSH)SelectObject(hGraphicsDC,
...
void drawellipticarc(float xc, float yc, float radx, float rady, float startang,
    ...
    if (disp_type == SCREEN) {
        ...
        if (angextent > 0) {
            p1 = (int)(xcoord(xc) +
                       fabs(xmult * radx) * cos(DEGTORAD(startang)));
            p2 = (int)(ycoord(yc) -
                       fabs(ymult * rady) * sin(DEGTORAD(startang)));
            p3 = (int)(xcoord(xc) +
                       fabs(xmult * radx) *
        ...
        fprintf(ps, "0 0 %.2f %.2f %.2f %s\n", /*XPOST(xc)*/
                /*YPOST(yc)*/ fabs(rady * ps_xmult), startang,
...
void fillellipticarc(float xc, float yc, float radx, float rady, float startang,
    ...
    if (disp_type == SCREEN) {
        ...
        if (!(hOldBrush = (HBRUSH)SelectObject(hGraphicsDC, hGraphicsBrush)))
            SELECT_ERROR();
        // Win32 API says a zero return value indicates an error, but it seems
        // to always return zero.  Don't check for an error on Pie.
        Pie(hGraphicsDC, xl, yt, xl + width, yt + height, p1, p2, p3, p4);

        //		if(!Pie(hGraphicsDC, xl, yt, xl+width, yt+height, p1, p2,
        //p3, p4)); 			DRAW_ERROR();
        if (!SelectObject(hGraphicsDC, hOldPen))
            SELECT_ERROR();
        if (!SelectObject(hGraphicsDC, hOldBrush))
...
#ifdef X11
        XFillPolygon(display, toplevel, current_gc, transpoints, npoints,
    ...
    while (corner < 2) {
        ...
        case MotionNotify:
            if (corner) {
                ...
                // won't update the drawing context if there is no change in
                // line width etc. (but they might only be on the normal
                // context)
...
#ifdef WIN32
        MessageBox(hMainWnd, "Error initializing postscript output.", NULL,
                   MB_OK);
#endif
    }
}
...
static void build_default_menu(void) {
    ...
    menu = XCreateSimpleWindow(display, toplevel, top_width - MWIDTH, 0, MWIDTH,
                               display_height, 0, colors[BLACK],
                               colors[LIGHTGREY]);
...
static void build_textarea(void) {
    ...
    textarea =
        XCreateSimpleWindow(display, toplevel, 0, top_height - T_AREA_HEIGHT,
                            display_width - MWIDTH, T_AREA_HEIGHT, 0,
...
static void drawmenu(void) {
    ...
    XSetForeground(display, gc_menus, colors[BLACK]);
    XDrawLine(display, menu, gc_menus, 0, top_height - 1, MWIDTH,
              top_height - 1);
...
static LRESULT CALLBACK GraphicsWND(HWND hwnd, UINT message, WPARAM wParam,
    ...
    case WM_CREATE:
        ...
        if (!SetMapMode(hAllObjtestDC, MM_TEXT))
            CREATE_ERROR();
        if (!(hbmAllObjtest = CreateCompatibleBitmap(
                  hAllObjtestDC, display_width, display_height)))
            CREATE_ERROR();
    ...
    case WM_PAINT:
        ...
        if (InEventLoop) {
            ...
            if (windowAdjustFlag > 1) {
                hDotPen =
                    CreatePen(PS_DASH, 1, win32_colors[background_cindex]);
                if (!hDotPen)
                    CREATE_ERROR();
                if (!SetROP2(hGraphicsDC, R2_XORPEN))
                    SELECT_ERROR();
                ...
                if (!SelectObject(hGraphicsDC, hDotPen))
                    SELECT_ERROR();
                if (!Rectangle(hGraphicsDC, oldAdjustRect.left,
                               oldAdjustRect.top, oldAdjustRect.right,
                               oldAdjustRect.bottom))
                    DRAW_ERROR();
    ...
    case WM_LBUTTONDOWN:
        if (!windowAdjustFlag) {
            ...
            // Special handling for the "Window" command, which takes multiple
            // clicks. First you push the button, then you click for one corner,
            // then you click for the other corner.
            if (windowAdjustFlag == 1) {
                windowAdjustFlag++;
    ...
    case WM_RBUTTONDOWN:
        if (!windowAdjustFlag) {
            ...
            for (i = 0; i < num_buttons; i++) {
                ...
                SendMessage(button[i].hwnd, BM_SETSTATE, button[i].ispressed,
                            0);
            }
    ...
    case WM_COMMAND:
        if (!windowAdjustFlag) {
            ...
            if (windowAdjustFlag) {
                ...
                for (i = 0; i < num_buttons; i++) {
                    EnableWindow(button[i].hwnd, FALSE);
                    SendMessage(button[i].hwnd, BM_SETSTATE,
...
/* implement X11 version here */
        ...
        if (!fill) {
            ...
            if (!(hOldPen = (HPEN)SelectObject(hGraphicsDC,
                                               GetStockObject(NULL_PEN))))
                SELECT_ERROR();
            if (!(hOldBrush =
                      (HBRUSH)SelectObject(hGraphicsDC, hGraphicsBrush)))
                SELECT_ERROR();
        }
        ...
        for (i = 1; i < npoints; i += 3)
            fprintf(ps, "%.2f %.2f %.2f %.2f %.2f %.2f curveto\n",
                    XPOST(points[i].x), YPOST(points[i].y),
                    XPOST(points[i + 1].x), YPOST(points[i + 1].y),
                    XPOST(points[i + 2].x), YPOST(points[i + 2].y));
        if (!fill)
            fprintf(ps, "stroke\n");
        else
            fprintf(ps, "fill\n");
...
int findfontsize(float ymax) {
    // find the correct point size which will fit in the specified ymax as the
    // max height of the font, using a binary search
    int bot = 1;
